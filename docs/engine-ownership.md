...
- **Where an entry may anchor.** `engineAnchorPrices()` (`lib/analyze/engineFacts.ts`) is the one
  ...
  session-VWAP rungs (feat-097) and the TIME-based value levels terrain can never mint —
  `tpo.poc`, `tpo.valueArea` and `multiDayTpo.composite.poc` (feat-090). Distinct from
  `engineZoneBorders()`, which is the hard-enforced zone stack the model must reproduce and
  therefore stays a strict partition read (a VWAP band or a POC is structure, not a partition —
  verified on the fixture bundle: 10 zones / 11 borders, unchanged by feat-109 — the VRange
  1x-zone merge only fires where BOTH edges promote, and on this fixture only the far edge has
  the volume geometry for it).
- **ATR is retired as an engine measure (feat-113).** The ATR-projected rung anchor class
  (feat-108, `lib/engine/atrProjection.ts`) is deleted: the engine carries ONE volatility
  measure, feat-095's Parkinson sigma, and a second one disagreeing with it in the same
  payload was the defect. The MGI `ATR High` / `ATR Low` levels are unaffected — those are the
  operator's own Sierra chart levels, exported data, not an engine computation. The NORMALIZER
  half (`htfStructure.atrPoints`, `rotation.extentAtr`, `currentVsSwings.*Atr` — feat-049) is
  still present and is feat-116's job to convert.
...
| `job_study_daily` / `job_study_weekly` | `job-study-daily.json` / `job-study-weekly.json` (two exporter studies, one per chart — split 2026-08-23) | Job planning task only (feat-121 plumbing; docs/job-planning-task-plan.md): parsed by the feat-125 parseJobStudy module and consumed by the feat-126/127 deterministic planner via the feat-128 job-plan task — NOT read by analyze/eval/update; the briefing engine pass never opens it | nothing — never enters the engine facts payload or any briefing/eval prompt; the Job planner (no LLM) reads the JBA pivots, value zones, target ladders, chart-drawn balance areas and Autoplot extremes directly |
| `five_day_vbp` | `five-day-rolling.vbp.md` | Job planning task only (feat-121 plumbing): rendered to an image by the feat-122 profile renderer for the feat-123 vision read (profile nodes) feeding the deterministic planner — NOT read by analyze/eval/update; parseProfile reads the format unchanged | nothing on the briefing side — the vision-read nodes reach only the Job plan (feat-128), never the engine facts payload |
| `four_hour_vbp` | `four-hour-rolling.vbp.md` | Job planning task only (feat-121 plumbing): same path as the 5-day profile — feat-122 render, feat-123 vision read, deterministic planner — NOT read by analyze/eval/update | nothing on the briefing side — Job plan only (feat-128) |

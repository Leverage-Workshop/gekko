# Trigger.dev Integration Rules

> Reference for implementing Trigger.dev tasks/schedules. Installed via `trigger.dev install-rules`
> and moved here out of `CLAUDE.md` to keep the harness lean. The Trigger.dev MCP server
> (named `trigger`, in `~/.claude.json`) provides live docs and run debugging.

...
const LOCAL_FILENAMES_BY_FIELD: Readonly<Record<string, readonly string[]>> = {
  ...
  // Job-planning inputs (feat-121): the names feat-118's Job exporters (daily/weekly,
  // one per chart) and the companion profile studies write. Absent until those
  // exporters are deployed — the uploader skips missing files, so bundles simply lack
  // the refs until then.
  job_study_daily: ['job-study-daily.json'],
  job_study_weekly: ['job-study-weekly.json'],

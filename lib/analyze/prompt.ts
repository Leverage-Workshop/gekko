...
export function buildAnalysisPrompt(input: AnalysisPromptInput): string {
  ...
  return [
    ...
    '- `fakeoutTails` is the code-owned FORMATION test on the rotation profile (feat-075): every High/Low-type MGI extreme (IB/session/prior-day/week/month highs and lows) whose print sits at the far end of a thin low-volume tail, with the `acceptanceEdge` LVN node where retests actually stall and the tail evidence (`tailSpanPts`, `maxTailBinFrac`). A listed extreme IS fakeout-formed — do not re-derive or dispute the finding, and do not counter it with balance-area/composite acceptance (the session-lens rule: the trade-horizon profile governs where retests stall). Anchoring a fade AT a listed extreme is allowed ONLY when the rationale states why the extreme beats the edge (a campaign border sits at the extreme, or initiative is actively repairing the tail); otherwise anchor at the listed `acceptanceEdge.price` and name the node. An empty list = no extreme currently carries a fakeout tail.',
    ...
    '- ACTIVE PATTERN SCAN (required): scan the execution chart for the doctrine playbook setups (Failed Breakout Trap, Controlled Flush & Reload, Three-Push Exhaustion, absorption or exhaustion at a border) and fill `patternScan`: verdict `present` (name the playbook pattern in `pattern`, say where it fired in `evidence`), `absent` (a readable chart with no playbook pattern), or `indeterminate` (the chart does not support a confident call — ambiguous shape, mid-formation, degraded image; say why in `evidence`). NEVER guess a pattern into existence: `indeterminate` is the honest verdict, and an indeterminate scan cannot serve as the visual evidence for an entry trigger — a pattern-justified trigger requires verdict `present`; otherwise ground the trigger in engine facts or abstain per the Objective contract. Mirror the verdict as one of `overview.current.keyPoints`.',
    '- Each of the three `overview` sections (`overview.htfView`, `overview.mtfView`, `overview.current`) is a `narrative` paragraph plus 2–4 `keyPoints` (the schema rejects fewer or more). The narrative is a TIME-ORDERED story of that timeframe with explicit time anchors, ending by setting up the section below; the keyPoints are the distilled takeaways the data surfaces, never a restatement of the narrative. ALL THREE sections carry the same storytelling register: `htfView` and `mtfView` must read like `current` does — a campaign told at named structure ("value collapsed through PW Low, then rebuilt above it"), NEVER a recitation of dates and bare POC prices. Call levels by their MGI/structural NAME (price at most in parens after the name); a day\'s value reference is named by its day ("Tuesday\'s POC (27600)"), the composite by its identity ("the 5-day composite POC (28054)"). The overview speaks in MGI and volume/TPO structure only — never terrain-zone names, never ATR (the Output Contract section carries the full vocabulary rules).',
    ...
    '- Each objective slot (primary AND secondary) carries EITHER a full trade — at least one entry, at least one stop on the protective side of that entry, and a target ladder per the Objective contract (T1 -> T2 when the map offers an intermediate rung, a single target labeled T2 when it does not) — OR the explicit no-trade abstention shape from the Objective contract (`noTrade: true` + `reasonCode`). The secondary is the best available counter-scenario; when it is real but waiting, express that in its entry `trigger` conditions — abstain only when no genuine scenario exists, and NEVER fabricate entries, stops or targets to fill the slot.',

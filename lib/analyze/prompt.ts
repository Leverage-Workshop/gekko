...
export function buildAnalysisPrompt(input: AnalysisPromptInput): string {
  ...
  return [
    ...
    '- BOTH objectives (primary AND secondary) must each carry at least one entry, at least one stop on the protective side of that entry, and a target ladder per the Objective contract: T1 -> T2 when the map offers an intermediate rung, a single target labeled T2 when it does not. The secondary is the best available counter-scenario; if it is not yet actionable, express that in its entry `trigger` conditions — never by omitting entries, stops or targets.',

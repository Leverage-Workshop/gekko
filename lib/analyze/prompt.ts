...
export function buildAnalysisPrompt(input: AnalysisPromptInput): string {
  ...
  return [
    ...
    '- Entries, stops and T1 must sit on engine-supplied structure — a zone border, a `terrain.levels` price, or a `lvnHvnNodes` LVN node price (taper-edge/valley) — never in the middle of value. The LVN-node anchor is reserved for the fakeout-formed-extreme case in the Objective contract: when you judge an MGI High/Low was printed by a pre-reversal fakeout, anchor at the LVN node marking where the actual action ended and name the node in the label. Entry priority, stop placement and the two-target T1 -> T2 ladder (T2 = the move\'s realistic conclusion, T1 = a structure rung near the middle of the entry->T2 traverse) follow the Objective contract in the system prompt.',

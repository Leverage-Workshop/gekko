 ...
 * ───────────────────────────────────────────────────────────────────────────
 *
 * The rest gets the same trim as `mgiPriority.tier1` (and as the ATR rungs did
 * before feat-113 retired them):
...
export const MGI_STRUCTURE_RULE =
  "- `mgiPriority` is the code-owned MGI structural read: `levels` carries every exported level with its tier and Daily MGI Priority rank, `tier1` the campaign-border subset price-descending, and `dailyPrioritySort` the daily group in Daily MGI Priority Order (1 Rip + Job Pivot · 2 ONH/ONL · 3 PDH/PDL · 4 RVAH/RVAL · 5 RPOC · 6 IBH/IBL · 7 VWAP). That ordering is RANK, not distance — it lists the Rip first even when price is 200 pts away from it and the OR levels last with price sitting on one — and levels sharing a rank (Rip/Job Pivot, ONH/ONL, PDH/PDL, IBH/IBL) are TIED, ordered by price alone with no importance implied. For distance read the `nearest*` fields instead: `nearestTier1Above/Below` are the nearest CAMPAIGN borders each side, and `nearestDailyAbove/Below` the nearest intraday daily levels each side — the Rip, PDH/PDL, IBH/IBL, RVAH/RVAL/RPOC and the OR levels can never surface in the Tier-1 read no matter how close they sit, so this is where the closest live session structure appears for nearest-first entry selection. `Job Pivot` and `Weekly Job Pivot` are the auction's LINE IN THE SAND — the level directional bias flips across depending on which side price can hold, and where large rotations visibly start and stop. Read the daily one as a bias filter alongside the Rip (price holding above it is the bullish case, below it the bearish one, and the side price cannot reclaim is the invalidation) and the weekly one as that same read at a weekly horizon, built from the PRIOR week's activity, which is why it tiers as a campaign border a session can spend the whole day working against. Price sitting ON a pivot is an undecided auction, not confirmation of either side. The VRange levels are IMPLIED-VOLATILITY projections off the session open (the Implied Vol Ranges study: the session open ± a multiple of the VIX-implied expected session move), so read them as expected-move geography, never as traded acceptance: `VRange Upper`/`VRange Lower` are the inner mean-reversion lines that behave like value-area edges and flip to support/resistance when broken with conviction, and the four `VRange 1x Zone` levels are the NEAR and FAR edges of ONE shaded zone marking the full expected session move — an inflection band price may not reach at all, and when it does, the zone is the object, not either edge alone. Terrain merges those two edges into one composite border; quote the band by name and take entry/stop prices from its member edges."
...
export function buildAnalysisPrompt(input: AnalysisPromptInput): string {
  ...
  return [
    ...
    "- `htfStructure` is the code-owned HTF structure read (feat-049), computed from the 30-min planning chart's bars: trend state from the confirmed swing sequence (`htfStructure.trend`), the recent swing highs/lows, the DEFINING rotation (`htfStructure.rotation`) and the measured 30-min ATR (`htfStructure.atrPoints`, with ATR-normalized distances in `htfStructure.currentVsSwings`). The rotation is the span between the most recent CONFIRMED swing high and low, which is NOT the same as the current range: each leg carries its bar time (`rotation.highDateTime` / `rotation.lowDateTime`) and the two legs need not come from the same session. NEVER call it \"the current rotation\" — date it when either leg predates the current session (\"the defining rotation, set overnight between the 23:30 low and the 06:30 high\") and say plainly where the live session has already traded outside it. The swing state is a LAGGING read (pivots confirm 2.5 h late) — `htfStructure.trend.integrity` (feat-064) squares it with the live price: 'intact', 'under-test' (counter-move retraced most of the defining rotation) or 'broken' (price has traded through the defining swing). NEVER state a directional HTF trend without its integrity qualifier — \"down, but broken in real time\" and \"down, intact\" are different battlefields. Ground meta.htfTrend and every HTF narrative in these numbers — the planning-chart screenshot adds distribution shape only, never the trend call. When `htfStructure` is null the bundle carried no HTF bar export — say so and mark the trend read as vision-only.",
    HTF_FLOW_RULE,

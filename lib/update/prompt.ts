...
export function buildUpdatePrompt(input: UpdatePromptInput): string {
  ...
  return [
    ...
    '- `fakeoutTails` is the code-owned FORMATION test on the rotation profile (feat-075): every High/Low-type MGI extreme whose print sits at the far end of a thin low-volume tail, with the `acceptanceEdge` LVN node where retests actually stall. A listed extreme IS fakeout-formed — do not re-derive or dispute the finding, and do not counter it with balance-area/composite acceptance. Anchoring a fade AT a listed extreme is allowed ONLY when the rationale states why the extreme beats the edge (a campaign border at the extreme, or initiative actively repairing the tail); otherwise anchor at the listed `acceptanceEdge.price` and name the node.',

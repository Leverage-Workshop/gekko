...
export const RULES: readonly Rule[] = [
  ...
  },
  {
    ...
    text: 'The two sides of the plan are the two sides of the FRAME, not of price. The BIAS side: with price above the line, the areas between the line and price are longs where a pullback will rebid, and so is the line itself. An area above price that has not been reached yet gets two reads: the long, where the trade is the hold after the break, never the break itself (price has to break it and hold to get in), and the short on a fail there (a JBA border overhead can also be a short). Trades at levels price has not reached are always one of those two — a fail against the line’s direction, or a breach-and-hold with it. The FORK side: what to do once price loses the line — the line itself becomes a reoffer on the pullback into it, then each significant level beyond it in turn, each conditional on the last (if price breaks the line, then another important level, the pullback into that level is the play). Mirror all of it with price below the line. Lead with the bias side. A side with nothing worth writing gets a one-line reason instead of a filler play.',
  },
  {

...
model only *where* a node sits (a normalized fraction, 0 = bottom edge of the image, 1 = top) and
let code turn that into price from the span the image is already known to cover. The pixel budget
is generous and was measured, not derived: on the 900x1400 render the R1 match tolerance
(ES 5 / NQ 20 pts) is 31–52 px, i.e. **2–4 % vertical accuracy**, not pixel accuracy.

**Shipped as a PARALLEL arm, never a replacement.** `config.profile_vision_model_id` is set, so the
price-axis path is live in production and stays byte-identical. Nothing about the axis variant
changed: the renderer's snapshot hash is unchanged, and `prompt.test.ts.snap` shows **insertions
only** — the axis-mode prompt text is identical to `vision-2026-08-30.7`'s.

| Piece | What changed |
|---|---|
| `renderProfile.ts` | `RenderOptions.axis` (default `true`) + `RenderMeta.axis`. `false` drops the axis line, ticks, price labels **and** the major gridlines (they exist only to carry the eye to a label), moves `plotRight` from `width-132` to `width-24` — bars 744 → 852 px, **+14.5 %** — and renders marker labels as bare tags, so no `<text>` on the image holds a digit. POC / VAH / VAL lines and the value-area shade stay: they are the only visible calibration anchors left. |
| `normalized.ts` (new) | `fractionToPrice` / `priceToFraction` / `toPriceRead` / `toNormalizedRead`. Fractions are stored to 6 dp — round-trip error `span × 5e-7`, four orders of magnitude inside the match tolerance. |
| `schema.ts` | `profileNodesReadNormalizedSchema` (the axis-free WIRE contract) and `profileNodesReadEitherSchema` (all four bound fields optional, exactly-one-pair enforced). All three reads share one `refineRead`, so the count / primary / rationale rules cannot drift apart. |
| `identifyProfileNodes.ts` | The conversion **boundary**: `runCall` validates against the schema the call asked for and converts fractions using `call.tile`'s span. `RawSample.read`, consensus, the bench scorer, the planner and the persisted `job_plans.profile_nodes` keep seeing prices only. |
| `prompt.ts` | `VISION_PROMPT_REVISION` → `vision-2026-08-30.8`. `ROLE_AXIS_FREE`, `OUTPUT_RULES_AXIS_FREE` and an `AXIS_FREE_NOTE` selected by `meta.axis` inside the **same** `buildVisionPrompt`. Per-call text gives POC / VAH / VAL / current as fractions alongside their prices. |
| few-shot | `manifest.json` gains `expectedNormalized`; the two files are generated by `scripts/few-shot-normalize.ts` (`--check` verifies) and pinned by a test that they equal `toNormalizedRead(expected, span)` and convert back to the price file's prices. |
| bench | `--variant axis-free` = `{ axis: false, tiles: 1 }`. |
| `scripts/render-profile.ts` | `--axis-free`, for eyeballing. |

**Two real bugs the bench would have hit on the axis-free arm, now fixed.**
1. `makeGenerate` **hardcoded** `schema: profileNodesReadSchema` and ignored the `schema` its caller
   passed. The axis-free arm would have asked the model for prices while the prompt asked for
   fractions. It now forwards the schema it is given.
2. The response cache validated every entry with `profileNodesReadSchema`, so a cached axis-free
   read would have been silently dropped and re-billed. It now validates with the either-schema and
   replays an entry only when it parses under the **current arm's** schema.

**Design decisions worth not re-litigating.**
- **Fraction, not pixel row.** Resolution-independent: a render-size change does not invalidate the
  few-shot examples or the cache.
- **Both wire schemas stay strict** (every field required, no optionals). A provider in strict
  structured-output mode wants every property in `required`; only the never-on-the-wire `either`
  schema uses optional fields. That is why there are three schemas rather than one permissive one.
- **Conversion against the TILE span, not the profile span**, so a tiled axis-free render converts
  against the window the model actually saw. Tested.
- **`axis-free` defaults to `tiles: 1`.** Tiling exists because a tall profile's axis text shrinks
  until it is illegible — and there is no axis text. The tiling machinery is untouched.
- **The marker labels keep their tag and lose their price.** Leaving "POC 25370.00" on the image
  would put a readable number back and hand the model a way to interpolate — the exact step this
  feature removes. The prompt states each anchor's fraction *and* its price instead.
- **`tileGeometry` reads `axis` loosely** (`meta.axis !== false`) because the dashboard overlay
  passes a `z.looseObject` parse of a persisted `RenderMeta`; a row written before feat-135 has no
  `axis` key and must map at the axis plot width.

**Risks / things the operator should know before the A/B.**
- **The revision bump costs a re-run of the price arm.** The bench cache keys on
  `VISION_PROMPT_REVISION`, so `.8` cold-starts every cached price read even though the axis-mode
  prompt text did not change by one byte. Running the axis arm on `.7` first (or `git stash`-ing the
  bump) would avoid re-paying; the bump was an explicit instruction, so it is in.
- **No live call has been made.** The normalized schema is untested against a real provider. The
  failure mode to watch for is a model that ignores the fraction range and returns prices anyway —
  that surfaces as a schema-validation failure per sample, with the zod message in
  `CaseResult.errors`, not as a bad read.
- **Fraction precision is not the risk; fraction *bias* is.** Round-tripping is exact to 0.001 pts.
  If the arm underperforms it will be because the model's sense of "40 % up the image" is
  systematically off (edge effects near y = 0 / 1, or anchoring on the POC line), not because a
  fraction cannot express the answer. Worth checking the sign of the residual per node position
  before concluding the idea does not work.
- Nothing here changes the `config` row, the DB, or the production render.

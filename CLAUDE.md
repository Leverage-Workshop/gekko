<!-- PROJECT HARNESS POINTER START -->
# Project Harness — start here

This repo builds **Ulysses** (advisory-only NQ-futures agent). Before doing project work:

1. Read **`AGENTS.md`** — the harness: startup workflow, one-feature-at-a-time rule, definition of done.
2. Read **`docs/agent-architecture-plan.md`** — the architecture and rationale (source of truth for *what*/*why*).
3. Read **`feature_list.json`** — the work breakdown; pick one feature whose dependencies are all `done`.
4. Run **`./init.sh`** before claiming anything done.

Everything below is **Trigger.dev integration guidance** (installed via `trigger.dev install-rules`).
Use it when implementing trigger.dev tasks/schedules. The Trigger.dev MCP server (named `trigger`,
registered in `~/.claude.json`) and the Vercel plugin provide live docs for those integrations.

---
<!-- PROJECT HARNESS POINTER END -->

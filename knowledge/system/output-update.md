...
- **`primary` / `secondary`** — fresh objectives, same `Objective` contract as a full briefing.
  The primary is INTRADAY-trend-aligned (`intradayTrend.direction`) per the Law of Asymmetric
  Initiative — never awarded off the lagging HTF swing state; the secondary is the
